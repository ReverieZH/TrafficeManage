   ...
   <%-- <style>
    ...
    <script>
        ...
        function changeStatus(username,Status) {
            ajaxRequest(username,Status);
        }

        function ajaxRequest( username,status) {
            var ajax;
            ...
            ajax.open("post", "/user/changeStatus.do");
        ...
        function deletajax( username ,obj) {
            var ajax;
            if(window.XMLHttpRequest){//火狐
                ajax=new XMLHttpRequest();
            }else if(window.ActiveXObject){//ie
    ...
    <script type="text/javascript">
        layui.use("table",function () {
               ...
               table.on('tool(headtoolbar)',function (obj) {
                   ...
                   switch (eventName) {
                       case "edit":
                                   getiframe('/user/geteditUser.do?username='+tr.username);
                                   break;
                        case "forbid":
                                   changeStatus(tr.username,0);
                                   break;
                        case "del":
                                  layer.confirm("您确认要删除吗?",function (index) {
                                        deletajax(tr.username,obj);
                                        layer.close(index);
                                  });
                                   break;
                   }
